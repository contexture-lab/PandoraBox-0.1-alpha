	...
	if (isset($signin)) {
		...
		if (!empty($login)) {
			...
			if ($res) {
				...
				$mailmessage = $mailsubject.' http://'.$_SERVER['HTTP_HOST'].'/?change='.$chpassw;
		...
		if ($password != $password_retry) {
			...
			if ($res1) {
				...
				$mailmessage = $mailsubject.' http://'.$_SERVER['HTTP_HOST'].'/?confirm='.$chpassw;
				$mailheaders = 'From: info@'.$_SERVER['HTTP_HOST']."\r\n".'Reply-To: info@'.$_SERVER['HTTP_HOST']."\r\n".'X-Mailer: PHP/'.phpversion();
		...
		if ($password == $password_retry) {
			...
			$regok = true;
			$errors[] = $locale['password_changed'].' <a href="http://'.$_SERVER['HTTP_HOST'].'/"> '.$locale['sign_in'].' </a> ';
			require 'signin.php';
		} else {
			$regok = true;
